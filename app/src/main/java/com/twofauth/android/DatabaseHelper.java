...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    private void onCreateV4(@NotNull final SQLiteDatabase database) {
        ...
        database.execSQL(String.format("CREATE TABLE %s (%s INTEGER PRIMARY KEY AUTOINCREMENT, %s TEXT, %s TEXT NOT NULL, %s TEXT NOT NULL, %s INTEGER, %s TEXT, %s TEXT, %s INTEGER DEFAULT 0, %s INTEGER DEFAULT %d, %s INTEGER DEFAULT %d, %s TEXT)", TwoFactorServerIdentitiesHelper.TABLE_NAME, TwoFactorServerIdentity.ROW_ID, TwoFactorServerIdentity.LABEL, TwoFactorServerIdentity.SERVER, TwoFactorServerIdentity.TOKEN, TwoFactorServerIdentity.REMOTE_ID, TwoFactorServerIdentity.NAME, TwoFactorServerIdentity.EMAIL, TwoFactorServerIdentity.IS_ADMIN, TwoFactorServerIdentity.SYNC_ON_STARTUP, resources.getBoolean(R.bool.sync_on_startup) ? 1 : 0, TwoFactorServerIdentity.SYNC_IMMEDIATELY, resources.getBoolean(R.bool.sync_immediately) ? 1 : 0, TwoFactorServerIdentity.SERVER_VERSION));
    ...
    public void onUpgrade(@NotNull final SQLiteDatabase database, final int old_version, final int new_version) {
        ...
        if (old_version < 5) {
            database.execSQL(String.format("ALTER TABLE %s ADD COLUMN %s TEXT", TwoFactorServerIdentitiesHelper.TABLE_NAME, TwoFactorServerIdentity.SERVER_VERSION));
        }
    }
