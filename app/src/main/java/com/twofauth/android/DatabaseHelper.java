...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    private void onCreateV1(@NotNull final SQLiteDatabase database) {
        ...
        database.execSQL(String.format("CREATE TABLE %s (%s INTEGER PRIMARY KEY AUTOINCREMENT, %s INTEGER NOT NULL REFERENCES %s(%s) ON DELETE RESTRICT, %s INTEGER DEFAULT %d, %s TEXT DEFAULT '')", TwoFactorGroupsHelper.TABLE_NAME, TwoFactorGroup.ROW_ID, TwoFactorGroup.SERVER_IDENTITY, TwoFactorServerIdentitiesHelper.TABLE_NAME, TwoFactorServerIdentity.ROW_ID, TwoFactorGroup.REMOTE_ID, 0, TwoFactorGroup.NAME));
