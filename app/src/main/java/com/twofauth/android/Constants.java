...
public class Constants {
    ...
    private static final String TWO_FACTOR_AUTH_ACCOUNT_UPDATED_DATA_PREFIX = "2fa-updated-data-";
    ...
    public static String getTwoFactorAccountUpdatedDataKey(JSONObject object) {
        return String.format("%s-%d", TWO_FACTOR_AUTH_ACCOUNT_UPDATED_DATA_PREFIX, object.optInt(TWO_FACTOR_AUTH_ACCOUNT_DATA_ID_KEY));
    }

    public static boolean theyAreTwoFactorAccountUpdatedData(@NotNull final Context context) {
        final SharedPreferences preferences = getDefaultSharedPreferences(context);
        for (final String key : preferences.getAll().keySet()) {
            if (key.startsWith(TWO_FACTOR_AUTH_ACCOUNT_UPDATED_DATA_PREFIX)) {
                return true;
            }
        }
        return false;
    ...
    public static SharedPreferences.Editor deleteTwoFactorAccountUpdatedDataKeys(@NotNull final SharedPreferences preferences, @Nullable SharedPreferences.Editor editor) {
        for (final String key : preferences.getAll().keySet()) {
            if (key.startsWith(TWO_FACTOR_AUTH_ACCOUNT_UPDATED_DATA_PREFIX)) {
                if (editor == null) {
    ...
    public static void deleteTwoFactorAccountUpdatedDataKeys(@NotNull final Context context) {
        final SharedPreferences.Editor editor = deleteTwoFactorAccountUpdatedDataKeys(getDefaultSharedPreferences(context), null);
        if (editor != null) {
            editor.apply();
        }
    }
