...
public class API {
    ...
    private static final String GET_API_VERSION = ICONS_LOCATION + "/packs";
    ...
    private static String getServerApiVersion(@NotNull final TwoFactorServerIdentity server_identity, @NotNull final Context context) {
        try {
            final HttpURLConnection connection = HTTP.get(new URL(GET_API_VERSION.replace("%SERVER%", server_identity.getServer())), AUTH_TOKEN.replace("%TOKEN%", server_identity.getToken()), areInsecureCertificatesAllowed(context));
    ...
    public static boolean refreshIdentityData(@NotNull final TwoFactorServerIdentity server_identity, @NotNull final Context context, final boolean raise_exception_on_error) throws Exception {
        ...
        if (connection.getResponseCode() == HttpURLConnection.HTTP_OK) {
            final String api_version = getServerApiVersion(server_identity, context);
            if (api_version != null) { server_identity.setApiVersion(api_version); }
